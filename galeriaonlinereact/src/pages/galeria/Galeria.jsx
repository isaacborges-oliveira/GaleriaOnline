...
export const Galeria = () => {
    ...
    async function cadastrarCard() {
        if (imagem && nomeImagem) {
            try {
                //FormData eh uma interface JavaScript que permite construir um conjunto de pares chave/valor 
                //representando os dados de um formulario html
            ...
            <div className="campoCards">
                {cards.length > 0 ? (
                    cards.map((e) => (
                        ...
                        <Card imagemCard={`https://localhost:7252/${e.caminho.replace("wwwroot/", "")}`}
                            tituloCard={e.nome}

                            funcEditar={() => editarCard(e.id)}
                            funcDeletar={() => deletarCard(e.id, e.nome)}
